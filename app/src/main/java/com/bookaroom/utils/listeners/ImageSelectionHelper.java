...
public class ImageSelectionHelper {
    ...
    public static void requestSelectionOrPermissions(Activity currentActivity, String selectImageMessage, int permissionsRequestCode, int pickImageRequestCode) {
        if (ActivityCompat.checkSelfPermission(currentActivity, Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
            requestSelection(currentActivity, selectImageMessage, pickImageRequestCode);
        }
        else {
            requestPermissions(currentActivity, permissionsRequestCode);
        }
    }

    private static void requestPermissions(Activity currentActivity, int permissionsRequestCode) {
    ...
    public static void requestSelection(Activity currentActivity, String selectImageMessage, int pickImageRequestCode) {
        ...
        currentActivity.startActivityForResult(chooserIntent, pickImageRequestCode);
    }
    ...
    public static Bitmap getSelectedImageBitmap(Activity currentActivity, Intent pickIntent) {
        Uri selectedImageUri = getSelectedImageUri(pickIntent);

        Bitmap selectedImageBitmap = null;
        try {
            selectedImageBitmap = MediaStore.Images.Media.getBitmap(currentActivity.getApplicationContext().getContentResolver(), selectedImageUri);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static SelectedImageInfo getSelectedImageInfo(Activity currentActivity, Intent pickIntent) {
        Uri selectedImageUri = getSelectedImageUri(pickIntent);
        ...
        Bitmap selectedImageBitmap = getSelectedImageBitmap(currentActivity, pickIntent);

        return new SelectedImageInfo(selectedImageUri, selectedImageBitmap, selectedImagePath);
    }
}
